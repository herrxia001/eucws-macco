...
if($year == "2023"){
	$thisDb_2 = new myDatabase($_SESSION['uDb']);	
	for ($i=0; $i<count($invs); $i++) {
		$sql = "UPDATE a_art_hist SET count = '".($invs[$i]['last'] + $invs[$i]['in_count'] - $invs[$i]['out_count'])."' WHERE ah_id = '".$invs[$i]['ah_id']."'";
		echo $sql."<br>";
		$thisDb_2->dbQuery($sql);
	}
}
?>
			...
			<div class="row">
				<div class="p-1 col-12 col-sm-12 col-md-12 col-lg-12">
					...
					<div class="input-group p-1">
						...
						<input type="number" min="-500" step="0.01" class="form-control" name="mda_dep_count" id="mda_dep_count">		
					</div>
					<div class="input-group p-1">
