...
//$sql = "SELECT a_invoice.* FROM a_invoice, customer WHERE a_invoice.k_id = customer.k_id AND customer.email IS NOT NULL AND customer.email != 'undefined' AND customer.email != '' AND xrechnung_st = '-1' AND date > '2025-01-01 00:00:00'";
$sql = "SELECT * FROM a_invoice WHERE r_id = '12359'";
...
foreach($result AS $order){
    ...
    if($myCustomer['email'] == "" || $myCustomer['email'] == "undefined"){
        continue;
    }else $recipient = $myCustomer['email'];

    //$recipient = "herrxia@gmail.com";
    ...
    foreach($a_in_items AS $orderItems){
        ...
        if($discount > 0){
            ...
            $pdf_priceStr = "<a style='text-decoration-line: line-through;'>".$orderItems['price']."</a> ".$priceStr;
		}
        $discount = 0;
...
$bodyHtml .= '<tr style="font-size:14px;">';
$bodyHtml .= '<td style="padding:1px;" align="right"><b>Total (inkl. MwSt):</b></td><td style="padding:1px;" align="right"><b style="font-size: 20px;">'.$order['net'].'&nbsp;&euro;&nbsp;</b></td>';
$bodyHtml .= '</tr>';
...
//sendEmailSmtp($recipient, $bodyHtml, $subject, $output, $pdf->Output('example_067.pdf','S'), $cc);
//sendEmailSmtp($recipient, utf8_decode($bodyHtml), $subject, $output, "", $cc);
    //$sql = "UPDATE a_invoice SET xrechnung_st = 1 WHERE r_id = '".$order['r_id']."'";
    //$thisDb->dbQuery($sql);
}
